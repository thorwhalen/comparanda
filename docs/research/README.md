...
| Is 2/3 the right coverage floor for matrices of *this* size? The sources are a 133-economy index; ours are ~22 x 12 | 2/3, provisionally — **not contradicted by a small simulation (2026-09-22, #84)** | Run: `scripts/coverage-floor-simulation.ts` over `languages.json` (4 × 5) and `relocation.json` (5 × 8) — far smaller than the ~22 × 12 the question is about, so pair counts re-sample few pairs (generated substitution weights — neither fixture carries any — and seeded completions of relocation's blanks). Share of truly ordered pairs the gated interval aggregate still orders: 100% at full coverage; 59–79% at [0.8, 1); 41–49% at [2/3, 0.8); 18–20% at [0.5, 2/3); 2–3% below 0.5. The renormalised point estimate the gate forbids reverses 7–13% of those pairs at [2/3, 1) and 20–32% below 2/3. The imputation column measures an estimate the gate forbids at every coverage below 1, so it does not locate the floor; the only cost of a lower floor is lost separability, which is still 18–20% just below 2/3. Nothing here argues for moving it, so 2/3 is kept as the default; it stays configurable, and the ADR's "provisional" label is Thor's to lift. |
